...
for data_name in ['CER', 'SAVE']:
    ...
    for SEED in range(10):
        for case_idx, n_sample_train in enumerate([20, 30, 40, 50, 100]):
...
feature_dict = dict()
feature_dict['SAVE_mrmr'] = mrmr_features_save
feature_dict['SAVE_all'] = np.arange(0, 48)

feature_dict['CER_mrmr'] = mrmr_features_cer
...
for feature_name in feature_dict.keys():
    ...
    X_train, X_test, y_train, y_test = train_test_split(data[:,valid_feature], label, test_size = 0.9, random_state = 0)
...
valid_index = [data + '_all_case_' + str(case) for case in range(5)]
self_index = [data + '_self_case_' + str(case) for case in range(5)]
...
# %%
result_ref = pd.read_csv(f'simulation_results/result_df_option_{option}_0926.csv')
